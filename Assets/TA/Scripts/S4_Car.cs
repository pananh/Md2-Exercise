...
public class S4_Car : MonoBehaviour
{
    private S4_Car instance;
    public S4_Car Instance
    {
        get
        {
            return instance;
    ...
    private int wayPointIndexCurrent;
    ...
    private void MoveCarAuto()
    {
        ...
        {
            direction = wayPoint[wayPointIndex[wayPointIndexCurrent]].transform.position - transform.position;
            MoveCar(direction.normalized * speedGame* 2 * Time.deltaTime);
        }
    ...
    {
        Debug.DrawLine(transform.position, transform.position + direction, Color.red,10f, true);
